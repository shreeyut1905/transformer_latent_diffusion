...
class DiffusionGenerator:
    ...
    def generate(
        ...
        if use_ddpm_plus:
            lambdas = [np.log((1-sigma)/sigma) for sigma in noise_levels]
        ...
        for i in tqdm(range(range(noise_levels)-1)):
...
class DiffusionTransformer:
    ...
    def generate_image_from_text(self,prompt:str,class_guidance=6,seed=11,num_imgs=1,img_size=32,n_iter=15):
        ...
        out , out_latent = self.diffuser.generate(
            labels,
            num_imgs = num_imgs,
            img_size = self.diffuer.model.image_size,
