...
class MHAttention(nn.Module):
    def __init__(self,is_casual=False,dropout_level=0.0,n_heads=4):
        super().__init__()
...
class SelfAttention(nn.Module):
    def __init__(self,embed_dim,is_casual=False,dropout_level=0.0,n_heads=4):
        super().__init__()
        self.qkv_linear = nn.Linear(embed_dim,3*embed_dim,bias=False)
        self.mha = MHAttention(is_casual,dropout_level,n_heads)
    def forward(self,x):
        q,k,v = self.qkv_linear(x).chunk(3,dim=2)
        return self.mha(q,k,v)

class CrossAttention(nn.Module):
    def __init__(self,embed_dim,is_casual=False,dropout_level=0,n_heads=4):
        super().__init__()
...
class MLP(nn.Module):
    def __init__(self,embed_dim,mlp_multiplier,dropout_level=0.0):
        super().__init__()
